...
public:
  ...
  const static int min_length = 31;
  ...
  void setDestinationAddr(const SmeAddress &p) {
    Header::updateLength(destination_addr.add(p));
  }

  void setDestinationAddr(const CString &p) {
    Header::updateLength(destination_addr.add(p));
  }
  ...
  const MultiDestinationAddresses &getDestinationAddr() const {
